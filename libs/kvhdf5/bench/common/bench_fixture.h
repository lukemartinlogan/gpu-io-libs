...
public:
    void SetUp(benchmark::State&) override {
        EnsureCteRuntime();
        size_t alloc_size = heap_size_ + 3 * hshm::ipc::kBackendHeaderSize;
        memory_ = new char[alloc_size];
        cuda::std::memset(memory_, 0, alloc_size);
    ...
    // Reset the arena allocator back to empty. Call between benchmark
    // iterations (inside state.PauseTiming()) when the benchmark creates
    // many short-lived objects that exhaust the bump allocator.
    void ResetAllocator() { allocator_->Reset(); }
