...
static BlobTestResult RunGpuBlobTest(
    ...
    GpuCteBlobStore store)
{
    GpuBlobBackends b = SetupGpuBlobBackends();
    BlobTestResult fail_result{-100, 0};
    if (!b.ok) return fail_result;

    chi::IpcManagerGpuInfo gpu_info = BuildGpuBlobInfo(b);

    BlobTestResult *d_result;
    cudaMallocHost(&d_result, sizeof(BlobTestResult));
    d_result->status = 0;
    d_result->data_match = 0;
    ...
    BlobTestResult result = *d_result;
    cudaFreeHost(d_result);
    hshm::GpuApi::DestroyStream(stream);
    return result;
}
...
TEST_CASE("GpuCteBlobStore - PutBlob + GetBlob roundtrip from kernel",
    ...
    GpuCteBlobStore store(tag_id, pool_id);
